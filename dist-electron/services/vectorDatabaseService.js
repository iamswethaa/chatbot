...
export class VectorDatabaseService {
     ...
     */
    async initializeIndex(dimension = 384) {
        try {
     ...
     */
    async searchDocuments(queryEmbedding, topK = 5, minScore = 0.7) {
        try {
            const index = this.pinecone.index(this.indexName);
     ...
     */
    async storeMessage(messageId, embedding, message, userId, sessionId) {
        try {
            const index = this.pinecone.index(this.indexName);
            const metadata = {
                messageId,
     ...
     */
    async searchSimilarMessages(queryEmbedding, topK = 5, userId, sessionId) {
        try {
            const index = this.pinecone.index(this.indexName);
            const filter = {};
            if (userId)
     ...
     */
    async getChatContext(queryEmbedding, maxContextLength = 3, userId, sessionId) {
        try {
            const results = await this.searchSimilarMessages(queryEmbedding, maxContextLength, userId, sessionId);
            return results
                .filter(result => result.score > 0.7) // Only include highly relevant messages
                .map(result => ({
                id: result.metadata.messageId,
                role: 'assistant', // Assuming we're retrieving assistant responses for context
    ...
    async deleteSession(sessionId) {
        try {
            ...
            const searchResults = await this.searchSimilarMessages(new Array(1536).fill(0), // Dummy vector for search
