...
export class SimpleDocumentProcessingService {
  ...
  constructor(pineconeApiKey: string, pineconeIndexName: string) {
   ...
   */
  private async processPDFFile(filename: string): Promise<void> {
    try {
   ...
   */
  private async processTextFile(filename: string): Promise<void> {
    try {
      const filePath = path.join(this.documentsPath, filename);
