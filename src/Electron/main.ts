...
let chatService: IntegratedChatService | null = null;
...
async function initializeChatService() {
  try {
    ...
    chatService = new IntegratedChatService(
      groqApiKey,
      pineconeApiKey,
      pineconeIndexName
    );
...
ipcMain.handle('chat:create-session', async (_event, userId?: string) => {
  try {
    if (!chatService) {
...
ipcMain.handle('chat:send-message', async (_event, message: string, sessionId: string, options = {}) => {
  try {
    if (!chatService) {
      throw new Error('Chat service not available');
    }
...
ipcMain.handle('chat:get-session', async (_event, sessionId: string) => {
  try {
    if (!chatService) {
      throw new Error('Chat service not available');
    }
...
ipcMain.handle('chat:stream-message', async (_event, message: string, sessionId: string, options = {}) => {
  try {
    if (!chatService) {
      throw new Error('Chat service not available');
    }
